...
// Mock Target Data (would come from API based on seller ID)
const MOCK_TARGET_DATA: TargetData = {
  monthlyTarget: 150000,
  quarterlyTarget: 450000,
  yearlyTarget: 1800000,
...
export default function DashboardPage() {
  ...
  const targetProgress = targetData && targetData.monthlyTarget > 0 
    ? Math.min((targetData.currentAchievement / targetData.monthlyTarget) * 100, 100)
    : 0;
  ...
  useEffect(() => {
    const fetchTargetData = async () => {
      ...
      try {
        ...
        setTargetData(MOCK_TARGET_DATA);
      } catch (err) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 p-3 sm:p-4 md:p-6 lg:p-8 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="mb-4 sm:mb-5 lg:mb-6">
          <div className="bg-gradient-to-r from-emerald-50 to-teal-50 backdrop-blur-md rounded-xl sm:rounded-2xl p-4 sm:p-5 lg:p-6 shadow-lg border border-emerald-200/50 hover:shadow-xl transition-all duration-300">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4">
              <div className="flex items-start gap-3 sm:gap-4">
                <div className="bg-gradient-to-br from-emerald-500 to-teal-600 p-2 sm:p-2.5 lg:p-3 rounded-xl shadow-lg flex-shrink-0">
                ...
                <div>
                  ...
                  {targetLoading ? (
                    <div className="flex items-center gap-2 mt-1">
                      ...
                      <span className="text-xs text-gray-500">Loading target data...</span>
                    </div>
                  ) : targetData ? (
                    <div className="space-y-1 mt-1">
                      <div className="flex flex-wrap items-center gap-2 sm:gap-3">
                        <span className="text-xs sm:text-sm text-gray-600">
                          <span className="font-semibold text-emerald-600">৳{targetData.currentAchievement.toLocaleString()}</span>
                          {" / "}
                          <span className="font-semibold text-gray-700">৳{targetData.monthlyTarget.toLocaleString()}</span>
                        </span>
                        ...
                        <span className={`text-[10px] sm:text-xs px-1.5 sm:px-2 py-0.5 rounded-full font-medium ${
                          targetData.priority === 'high' 
                            ? 'bg-red-100 text-red-700' 
                            : targetData.priority === 'medium' 
                            ? 'bg-amber-100 text-amber-700' 
                            : 'bg-green-100 text-green-700'
                        }`}>
                          {targetData.priority.charAt(0).toUpperCase() + targetData.priority.slice(1)} Priority
                        </span>
                      </div>
                      <div className="flex flex-wrap items-center gap-1 sm:gap-2 text-[10px] sm:text-xs text-gray-500">
                        <Calendar className="h-3 w-3 sm:h-3.5 sm:w-3.5" />
                        <span>
                          {new Date(targetData.startDate).toLocaleDateString()} - {new Date(targetData.endDate).toLocaleDateString()}
                        </span>
                        {targetData.notes && (
                          <>
                            <span className="w-1 h-1 bg-gray-300 rounded-full"></span>
                            <span className="truncate max-w-[150px] sm:max-w-[200px]">{targetData.notes}</span>
                          </>
                        )}
                      </div>
                    </div>
                  ) : (
                    <p className="text-xs text-gray-500 mt-1">No target set for this period</p>
                  )}
                </div>
              </div>
              <div className="w-full sm:w-auto">
                <div className="w-full sm:w-48 h-2 sm:h-2.5 bg-gray-200/70 rounded-full overflow-hidden">
                  <div 
