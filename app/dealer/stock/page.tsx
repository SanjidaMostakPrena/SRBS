...
const StatCard: React.FC<{
  ...
  return (
    <div className="group relative overflow-hidden rounded-xl md:rounded-2xl border border-white/50 bg-white/70 p-3 md:p-5 shadow-lg backdrop-blur-xl transition-all duration-300 hover:scale-[1.02] hover:shadow-2xl">
      <div className="absolute -top-10 -right-10 h-32 w-32 rounded-full bg-gradient-to-br from-blue-500/10 to-transparent opacity-0 transition-opacity duration-300 group-hover:opacity-100" />
      <div className="relative flex items-start justify-between">
        <div className="flex-1 min-w-0">
          <p className="text-[10px] md:text-xs font-medium tracking-wider text-gray-500 uppercase truncate">
            {label}
          </p>
          <p className="mt-1 md:mt-2 text-lg md:text-2xl font-bold text-gray-800 truncate">{value}</p>
          {subtitle && <p className="mt-0.5 md:mt-1 text-[10px] md:text-xs text-gray-400 truncate">{subtitle}</p>}
        </div>
        <div className={`flex-shrink-0 rounded-xl bg-gradient-to-br ${colorMap[color]} p-2 md:p-3 text-white shadow-lg`}>
          {icon}
        </div>
      </div>
    </div>
...
export default function DealerStockPage() {
  ...
  const getCategoryIcon = (category: string) => {
    switch (category) {
      case "paint":
        return <Paintbrush className="h-3 w-3 md:h-4 md:w-4" />
      case "admixture":
        return <Droplet className="h-3 w-3 md:h-4 md:w-4" />
      default:
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50/30 p-3 md:p-6 lg:p-8">
      ...
      <div className="relative z-10 mx-auto max-w-7xl">
        {/* Header */}
        <div className="mb-4 md:mb-8 overflow-hidden rounded-2xl md:rounded-3xl border border-white/40 bg-gradient-to-r from-blue-600/10 via-indigo-600/10 to-purple-600/10 p-4 md:p-6 lg:p-8 shadow-xl backdrop-blur-xl">
            ...
            <div className="flex items-center gap-3 md:gap-4">
              <div className="relative flex-shrink-0">
                <div className="rounded-xl md:rounded-2xl bg-gradient-to-br from-blue-600 to-indigo-700 p-2.5 md:p-3.5 text-white shadow-lg">
                  <Box className="h-5 w-5 md:h-7 md:w-8" />
                </div>
                <div className="absolute -right-1 -bottom-1 h-2.5 w-2.5 md:h-3 md:w-3 rounded-full bg-green-400 ring-2 ring-white" />
              </div>
          ...
          {/* Stats Grid */}
          <div className="mt-4 md:mt-6 grid grid-cols-2 gap-2 md:gap-3 sm:grid-cols-2 lg:grid-cols-4">
            <StatCard
              label="Total Products"
              value={stats.totalProducts}
            ...
            <StatCard
              label="Total Quantity"
              value={stats.totalQuantity}
              icon={<Layers className="h-3.5 w-3.5 md:h-5 md:w-5" />}
              color="teal"
              subtitle={`${stats.totalQuantity} units`}
            />
            <StatCard
        ...
        <div className="mb-4 md:mb-6 rounded-xl md:rounded-2xl border border-white/50 bg-white/70 p-3 md:p-4 lg:p-6 shadow-2xl backdrop-blur-xl">
          <div className="flex flex-col gap-2 md:flex-row md:items-center md:gap-3 lg:gap-4">
            <div className="relative flex-1">
              ...
              <input
                type="text"
                placeholder="Search by product name, code, brand or location..."
                value={searchTerm}
        ...
        {/* Stock Items */}
        {filteredItems.length === 0 ? (
          <div className="rounded-xl md:rounded-2xl border border-white/50 bg-white/70 p-8 md:p-12 text-center shadow-2xl backdrop-blur-xl">
            <Box className="mx-auto mb-3 h-10 w-10 md:h-12 md:w-12 text-gray-300" />
            <p className="text-sm md:text-base text-gray-500">No stock items found</p>
          ...
          <div className="overflow-hidden rounded-xl md:rounded-2xl border border-white/50 bg-white/70 shadow-2xl backdrop-blur-xl">
            <div className="overflow-x-auto">
              <table className="w-full text-xs md:text-sm">
                <thead className="border-b border-gray-200/50 bg-gradient-to-r from-blue-50/50 to-indigo-50/50">
                  <tr>
                    <th className="px-2 md:px-4 py-2 md:py-3 text-left font-semibold text-gray-700">
                      Product
                    </th>
                    <th className="hidden px-2 md:px-4 py-2 md:py-3 text-left font-semibold text-gray-700 md:table-cell">
                      Code
                    </th>
                    <th className="hidden px-2 md:px-4 py-2 md:py-3 text-left font-semibold text-gray-700 lg:table-cell">
                      Category
                    </th>
                    <th className="hidden sm:table-cell px-2 md:px-4 py-2 md:py-3 text-right font-semibold text-gray-700">
                      Buying Price
                    </th>
                    <th className="px-2 md:px-4 py-2 md:py-3 text-right font-semibold text-gray-700">
                      Selling Price
                    </th>
                    <th className="px-2 md:px-4 py-2 md:py-3 text-right font-semibold text-gray-700">
                      Quantity
                ...
                <tbody>
                  {filteredItems.map((item) => (
                    <tr
                      key={item.id}
                      className="border-b border-gray-100/60 transition hover:bg-white/30 cursor-pointer"
                      onClick={() => handleViewProduct(item)}
                    >
                      <td className="px-2 md:px-4 py-2 md:py-3">
                        <div>
                          <p className="text-[10px] md:text-sm font-medium text-gray-800 truncate max-w-[120px] md:max-w-none">
                            {item.name}
                          </p>
                          <p className="text-[8px] md:text-xs text-gray-500 truncate">{item.brand}</p>
                        </div>
                      </td>
                      <td className="hidden px-2 md:px-4 py-2 md:py-3 md:table-cell">
                        <span className="font-mono text-[10px] md:text-xs text-gray-500">
                          {item.code}
                        </span>
                      </td>
                      <td className="hidden px-2 md:px-4 py-2 md:py-3 lg:table-cell">
                        <span className={`inline-flex items-center gap-1 rounded-full px-2 md:px-3 py-0.5 md:py-1 text-[8px] md:text-xs font-medium ${getCategoryColor(item.category)}`}>
                          {getCategoryIcon(item.category)}
                          {item.category}
                        </span>
                      </td>
                      <td className="hidden sm:table-cell px-2 md:px-4 py-2 md:py-3 text-right">
                      ...
                      <td className="px-2 md:px-4 py-2 md:py-3 text-right">
                        <p className={`text-[10px] md:text-sm font-bold ${item.quantity === 0 ? 'text-red-600' : item.quantity < item.minStock ? 'text-yellow-600' : 'text-gray-800'}`}>
                          {item.quantity}
                        </p>
                        <p className="text-[8px] md:text-xs text-gray-400">{item.unit}</p>
                      </td>
                    </tr>
                  ))}
                </tbody>
      ...
      {/* Stock Detail Modal - Fully Responsive */}
      {showStockModal && selectedProduct && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-2 md:p-4 backdrop-blur-sm">
          <div className="max-h-[95vh] md:max-h-[90vh] w-full max-w-3xl overflow-y-auto rounded-xl md:rounded-2xl border border-white/50 bg-white/95 p-3 md:p-6 lg:p-8 shadow-2xl backdrop-blur-xl">
            <div className="mb-4 md:mb-6 flex items-start justify-between border-b border-gray-200 pb-3 md:pb-4">
              <div className="min-w-0">
                <div className="flex flex-wrap items-center gap-2">
                  <h2 className="text-base md:text-xl lg:text-2xl font-bold text-gray-800 truncate">
                    Stock Details
                  </h2>
                </div>
                <p className="mt-0.5 md:mt-1 font-mono text-[10px] md:text-sm text-gray-500">
                  {selectedProduct.code}
                </p>
              </div>
              <button
            ...
            <div className="grid gap-3 md:gap-4 grid-cols-1 md:grid-cols-2">
              <div className="rounded-lg md:rounded-xl border border-gray-200 bg-gradient-to-br from-blue-50/50 to-indigo-50/50 p-3 md:p-4 backdrop-blur-sm">
                ...
                <div className="space-y-1.5 md:space-y-2">
                  <p className="flex items-start gap-1.5 md:gap-2 text-xs md:text-sm">
                    <Tag className="mt-0.5 h-3 w-3 md:h-4 md:w-4 text-gray-400 flex-shrink-0" />
                    <span className="break-words">{selectedProduct.name}</span>
                  </p>
                  <p className="flex items-start gap-1.5 md:gap-2 text-xs md:text-sm">
                    <Building2 className="mt-0.5 h-3 w-3 md:h-4 md:w-4 text-gray-400 flex-shrink-0" />
                    <span className="break-words">{selectedProduct.brand}</span>
                  </p>
                  <p className="flex items-start gap-1.5 md:gap-2 text-xs md:text-sm">
                    <Box className="mt-0.5 h-3 w-3 md:h-4 md:w-4 text-gray-400 flex-shrink-0" />
                    <span className="break-words">{selectedProduct.packSize}</span>
                  </p>
                  <p className="flex items-start gap-1.5 md:gap-2 text-xs md:text-sm">
                    <MapPin className="mt-0.5 h-3 w-3 md:h-4 md:w-4 text-gray-400 flex-shrink-0" />
                    <span className="break-words">{selectedProduct.location}</span>
                  </p>
                  <p className="flex items-start gap-1.5 md:gap-2 text-xs md:text-sm">
                    <span className={`inline-flex items-center gap-1 rounded-full px-2 md:px-3 py-0.5 md:py-1 text-[8px] md:text-xs font-medium ${getCategoryColor(selectedProduct.category)}`}>
              ...
              <div className="rounded-lg md:rounded-xl border border-gray-200 bg-gradient-to-br from-purple-50/50 to-pink-50/50 p-3 md:p-4 backdrop-blur-sm">
                <h3 className="mb-2 md:mb-3 flex items-center gap-1.5 md:gap-2 text-xs md:text-sm font-semibold text-gray-700">
                  <BarChart3 className="h-3.5 w-3.5 md:h-4 md:w-4 text-blue-600" />
                  Stock Information
                </h3>
                <div className="space-y-1.5 md:space-y-2">
                  <p className="flex items-start gap-1.5 md:gap-2 text-xs md:text-sm">
                    <Package className="mt-0.5 h-3 w-3 md:h-4 md:w-4 text-gray-400 flex-shrink-0" />
                    <span className="break-words">
                      Quantity: <strong className={selectedProduct.quantity === 0 ? 'text-red-600' : selectedProduct.quantity < selectedProduct.minStock ? 'text-yellow-600' : 'text-gray-800'}>{selectedProduct.quantity}</strong>{" "}
                      {selectedProduct.unit}
                    </span>
                  </p>
                  <p className="flex items-start gap-1.5 md:gap-2 text-xs md:text-sm">
                    <DollarSign className="mt-0.5 h-3 w-3 md:h-4 md:w-4 text-gray-400 flex-shrink-0" />
                    <span className="break-words">
                      Selling Price: ৳
                      {selectedProduct.sellingPrice.toLocaleString()}
                    </span>
                  ...
                  <p className="flex items-start gap-1.5 md:gap-2 text-xs md:text-sm">
                    <AlertTriangle className="mt-0.5 h-3 w-3 md:h-4 md:w-4 text-gray-400 flex-shrink-0" />
                    <span className="break-words">
                      Min Stock: {selectedProduct.minStock}{" "}
                      {selectedProduct.unit}
                    </span>
                  ...
                  <p className="flex items-start gap-1.5 md:gap-2 text-xs md:text-sm">
                    <TrendingUp className="mt-0.5 h-3 w-3 md:h-4 md:w-4 text-gray-400 flex-shrink-0" />
                    <span className="break-words">
                      Max Stock: {selectedProduct.maxStock}{" "}
                      {selectedProduct.unit}
                    </span>
            ...
            <div className="mt-4 md:mt-6 flex flex-col gap-2 md:gap-3 border-t border-gray-200 pt-3 md:pt-4 sm:flex-row sm:justify-end">
              <button
                onClick={handleCloseModal}
              ...
              <button className="flex items-center justify-center gap-1.5 md:gap-2 rounded-lg md:rounded-xl bg-gradient-to-r from-blue-600 to-indigo-700 px-3 md:px-4 py-1.5 md:py-2 text-xs md:text-sm font-medium text-white shadow-lg transition hover:scale-105 hover:shadow-xl order-1 sm:order-2">
